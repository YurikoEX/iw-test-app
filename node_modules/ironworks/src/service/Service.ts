...
class Service extends Worker implements IService {
    ...
    private readyWorker(worker: IWorkerChild, workerStarted?: (e: Error) => void) {
        ...
        async.waterfall([
            ...
            (workerDeps, allDepsStarted) => {
                ...
                async.whilst(() => {
                    return !_.every(workerDeps, (w: IWorkerChild) => {
                        return w.running;
                    });
                }, (tryAgain) => {
            ...
            (workerDeps, workerReady) => {
                var deps = new Collection<IDependency<IWorkerChild>>(idHelper.newId());
                deps.addMany(workerDeps);
                worker.start(deps, (e: Error) => {
                    if (e === null) {
                        worker.running = true;
                    }
                    workerReady(e);
    ...
    public get(workerQuery: IWhoQuery, callback: (e: Error, results: IWorkerChild[]) => void): IService {
